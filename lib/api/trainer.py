...
from .eval_hooks import DistEvalMeanLossHook
...
def _dist_train(model, dataset, validate, cfg, logger):
    ...
    model = MMDistributedDataParallel(model, device_ids=[rank % num_gpus])
    # build runner
    ...
    if validate:
        ...
        # runner.register_hook(DistEvalMeanLossHook(val_dataset, cfg, interval=interval))
    if cfg.resume_from:
